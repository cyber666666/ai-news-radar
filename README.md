...
2. **开 Actions**：fork 后 GitHub 默认暂停 workflow，去 Actions 页点一下启用，`update-news.yml` 每 4 小时自动跑。
...
- 默认每 4 小时整点运行一次：`0 */4 * * *`（GitHub cron 使用 UTC；换算到北京时间仍为 00:00/04:00/08:00/12:00/16:00/20:00）
