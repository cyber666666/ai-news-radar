...
系统每 4 小时自动抓取过去一段时间的 AI 资讯,来源包括官方源、AI 媒体、RSS 订阅,以及三个**付费社交源**(X、抖音、小红书)。当前生效的关键设置:

| 维度 | 当前值 | 含义 |
|---|---|---|
| 运行频率 | **每 4 小时** | GitHub Actions 定时任务 |
...
| 参数 | 当前值 | 改哪里 |
|---|---|---|
| 运行频率 | 每 4 小时 | `.github/workflows/update-news.yml` → `cron: "0 */4 * * *"` |
...
> 付费源虽然每 4 小时有机会跑,但「24 小时间隔门」保证每天只真正花一次钱。
